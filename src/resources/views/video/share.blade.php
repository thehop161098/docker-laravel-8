...
<body>
    ...
    <section class="main wp-share-movie">
        <div class="wp-content">
            <form method="POST" action="{{ route('share-movie') }}">
                @csrf
                <fieldset>
                    <legend>Share a movie</legend>
                    <div class="wp-share-information">
                        <div>Youtube URL: <input type="text" name="youtube_url" value="{{ old('youtube_url') }}"></div>
                        @if ($errors->has('youtube_url'))
                            <div class="error-message">{{ $errors->first('youtube_url') }}</div>
                        @endif
